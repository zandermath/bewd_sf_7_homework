...
def list_apartments(list)
  ...
  # puts
  list.each do |hash|
    puts "Apartment: #{hash[:apt_name]}"
    puts " sqft: #{hash[:apt_sqft]}      Bedrooms: #{hash[:apt_beds]}      Bathrooms: #{hash[:apt_baths]}"
    puts "#{hash[:rented]}"
  end
end
...
0.upto(apt_count - 1) do |unit|
  ...
  apartment.unit_rented = gets.strip
...
if response == 'v'
  list = building.view_apartments
  list_apartments(list)
